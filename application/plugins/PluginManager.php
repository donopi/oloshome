...
class PluginManager extends Component implements BootstrapInterface
{
    ...
    public function enablePlugin($pluginId)
    {
        ...
        $plugin = $availablePlugins[$pluginId];
        try {
    ...
    public function getAvailablePlugins(): array
    {
        ...
        foreach ($subDirectories as $directory) {
            ...
            $plugin = $this->getPlugin($pluginId, $this->getPluginInfo($directory . '/plugin.json'));
            if ($plugin !== null) {
    ...
    public function getEnabledPlugins()
    {
        ...
        foreach ($pluginsRegistry as $pluginId => $pluginRegistry) {
            $plugin = $this->getPlugin($pluginId, $this->getPluginInfo($this->getPluginPath($pluginId) . '/plugin.json'));
            if ($plugin !== null) {
    ...
    private function getPlugin($pluginId, $pluginInfo)
    {
        ...
        $pluginClass = $pluginInfo['class'] ?? false;
        if (!$pluginClass) {
     ...
     * @return array|mixed
     */
    private function getPluginInfo($pluginInfoFile)
    {
        if (is_file($pluginInfoFile)) {
            try {
                $pluginJson = file_get_contents($pluginInfoFile);
                return json_decode($pluginJson, true);
            } catch (\Exception $exception) {
